...
class Mrp_sup extends CI_Model
{
    ...
    function json() {
        ...
        $wh=" a.kd_pengguna=$ku  AND tanggal BETWEEN '$date1' AND '$date2' and status='Tervalidasi'";
        $this->datatables->select("b.kode_supplier, b.nama_supplier");
        ...
        $this->datatables->group_by('nama_supplier');
        $this->datatables->add_column('action', '<div class="btn-group">'.anchor(site_url('pembelian/rp_sup/detail/$1'),'<i class="fa fa-binoculars"></i>','class="btn btn-xs btn-success"').'</div>', 'kode_supplier');
