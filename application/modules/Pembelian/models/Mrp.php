...
class Mrp extends CI_Model
{
    ...
    function json() {
        ...
        $wh="(c.panjang_kayu=".$p1." OR c.panjang_kayu=".$p2.") and a.jenis_kayu='$jenis_kayu'  and a.kd_pengguna=$ku  AND tanggal BETWEEN '$date1' AND '$date2' and status='Tervalidasi'";}
        else {$wh="(c.panjang_kayu=".$p1." OR c.panjang_kayu=".$p2.") and a.jenis_kayu='$jenis_kayu'    AND tanggal BETWEEN '$date1' AND '$date2' and status='Tervalidasi'";}
        $this->datatables->select("b.kode_supplier,DATE_FORMAT(a.tanggal, '%d-%m-%Y') as tgl,DATE_FORMAT(a.tanggal, '%d-%m-%Y') as tgl_val, b.nama_supplier, REPLACE(SUM(ROUND(c.volume,4)),'.',',') AS volume, SUM(c.harga) AS harga");
        ...
        $this->datatables->add_column('action', '<div class="btn-group">'.anchor(site_url('pembelian/rp/pdfrp/$1'),'<i class="fa fa-file-pdf-o"></i>','class="btn btn-xs btn-danger"').'</div>', 'kode_supplier');
