...
class Mlap_depo_sup extends CI_Model
{
    ...
    function json() {
        ...
        $wh=" a.kd_pengguna=$ku AND a.kode_supplier=$supplier AND tanggal between '$date1' and '$date2' and status='Tervalidasi'";}
        else {$wh=" a.kode_supplier=$supplier AND tanggal between '$date1' and '$date2' and status='Tervalidasi'";}
        $this->datatables->select("a.id_dukb,CONCAT(SUBSTRING(a.tanggal,9,2),'-',SUBSTRING(a.tanggal,6,2),'-',SUBSTRING(a.tanggal,1,4)) AS tanggal,CONCAT(SUBSTRING(a.tgl_validasi,9,2),'-',SUBSTRING(a.tgl_validasi,6,2),'-',SUBSTRING(a.tgl_validasi,1,4)) AS tanggal_val,b.nama_supplier,a.plat_nomor,a.jenis_kayu,a.kabupaten,a.kecamatan,a.kd_siklus,a.status,SUM(c.harga) AS harga, REPLACE(SUM(ROUND(c.volume,4)),'.',',') AS volume");
        ...
        $this->datatables->group_by('a.id_dukb');$this->db->order_by("a.id_dukb", "desc");
        $this->datatables->where($wh);
        $this->datatables->add_column('action', '<div class="btn-group">'.anchor(site_url('pembelian/lap_depo_sup/detail/$1'),'<i class="fa fa-binoculars"></i>','class="btn btn-xs btn-success"').'</div>', 'id_dukb');
    ...
    function getlist($supplier,$date1,$date2){
