...
WHERE (b.panjang_kayu=".$p1." OR b.panjang_kayu=".$p2.") AND kode_supplier='".$id."' AND tanggal BETWEEN '".$this->session->userdata('date1')."' AND '".$this->session->userdata('date2')."' AND a.status='Tervalidasi' AND jenis_kayu='".$this->session->userdata('jenis_kayu')."' GROUP BY a.id_dukb")->result(); foreach ($sql as $sql) { $total+=$sql->harga;
  ?>
                     <tr>
