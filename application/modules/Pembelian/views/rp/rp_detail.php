                      ...
                      <thead align="center">
                        <tr><?php $row=$this->db->query("select nama_supplier from ref_supplier where kode_supplier='".$id."'")->row();$nama=$row->nama_supplier;
                         ?>
                          ...
                          <td class="text-left" width="50%"><b>:</b> <?php echo $nama; ?></td>                         
                        </tr>
                        <tr>
...
WHERE (b.panjang_kayu=".$p1." OR b.panjang_kayu=".$p2.") AND tanggal='".$this->session->userdata('date')."' AND a.status='Tervalidasi' AND jenis_kayu='".$this->session->userdata('jenis_kayu')."' GROUP BY a.id_dukb")->result(); foreach ($sql as $sql) { $total+=$sql->harga;
  ?>
                     <tr>
